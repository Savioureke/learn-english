...
import { 
  Video, Calendar, BookOpen, Clock, Award, Shield, User, 
  Star, Gift, HelpCircle, Check, Play, ExternalLink, CheckCircle2,
  ChevronRight, Sparkles, Send, Mail, Phone, RefreshCw
} from 'lucide-react';
...
export default function StudentDashboard({ loggedInUser, onOpenAuth }) {
  ...
  const [activeTab, setActiveTab] = useState(initialTab); // 'tutors' | 'bookings' | 'courses'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gradient-to-r from-navy via-navy-light to-navy text-white rounded-3xl p-6 sm:p-10 shadow-xl mb-8 relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-white/15">
            ...
            <div className="bg-white/10 rounded-2xl p-3.5 backdrop-blur-xs">
              <div className="text-xs text-gray-300 font-heading">Self-Paced Course</div>
        ...
        <div className="flex overflow-x-auto scrollbar-none space-x-2 border-b border-gray-200 pb-2 mb-8">
          {[
            ...
            { id: 'courses', label: '3. 10-Module Spoken Fluency Course', icon: BookOpen },
          ].map((tab) => {
            const Icon = tab.icon;
        ...
        {activeTab === 'courses' && (
          <div className="space-y-6 animate-fadeIn">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-xs uppercase font-heading font-bold text-accent tracking-wider">
                  Self-Paced Mastery Course
                </span>
                <h2 className="text-2xl font-bold font-heading text-navy mt-1">
                  10-Module Spoken English & Fluency Curriculum
                </h2>
                <p className="text-xs sm:text-sm text-gray-500 mt-1">
                  Click any module below to open its full video lesson page, study key takeaways, and take the 3-question knowledge quiz.
                </p>
              </div>

              <span className="text-xs font-mono font-bold bg-navy text-white px-4 py-2 rounded-full">
                {completedModules.length} / 10 Modules Completed
              </span>
            </div>
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs bg-slate-100 text-gray-600 px-2.5 py-0.5 rounded-full font-medium flex items-center gap-1">
                          <Clock className="w-3.5 h-3.5 text-gray-400" /> {mod.duration}
                        </span>
                      </div>
                    ...
                    <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                      ...
                      <Link
                        to={`/student/module/${mod.number}`}
                        className="px-4 py-2 rounded-xl bg-navy hover:bg-navy-light text-white text-xs font-heading font-semibold flex items-center gap-1.5 transition-all shadow-xs"
                      >
                        <Play className="w-3.5 h-3.5 fill-current" />
                        <span>Open Lesson Page & Quiz →</span>
                      </Link>
                    </div>
                  </div>
                );
